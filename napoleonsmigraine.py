...
for line in linelist:
    ...
    # process line and remove unneeded mean value for year (last item in list)
        ...
        print('readingdict:', readingdict)
        print('newbarlist[r]', newbarlist[r])
        print('monthdict:', readingdict)

    # cur.execute('INSERT OR IGNORE INTO Stations (barlat, barlong) VALUES (?, ?)', (barlat, barlong))
    # cur.execute('INSERT OR IGNORE INTO Barometer (year, jan_read, feb_read, mar_read, apr_read,
    # may_read, jun_read, jul_read, aug_read, sep_read, oct_read, nov_read, dec_read) VALUES (?, ?, ?, ?
    # , ?, ?, ?, ?, ?, ?, ?, ?, ?)',
